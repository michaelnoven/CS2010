...
public class TSPSlow {
  ...
  private static void backtracking(int u) {
    ...
    if (all_visited) { // all V vertices have been visited, compute tour cost
      counter++;
//      if (counter % 1000 == 0)
//        System.out.println("We have examined = " + counter + " tours");
...
//      System.out.printf(", cost = %d\n", cost);
  ...
  // BUT if V = 2000, we get 8 billion.. C is too slow.
  // In C K is only 16. is like ps4+5. WE can run djikstras!! :) :) :)

  private static int memo[][] = new int[N][1 << N]; // vertex we are visiting + the vertex visited  (memo table), arraylist of integers will be slow.
                                                      // biggest value .. 1 << N equivalent to 2^N.. 1 shift 1 is 2. what is 2 power of 1 => 2. 
  


  private static int DP_TSP(int u, int m){

    // Base case 1, visited everything
    if(m == (1 << N) - 1) // if m is all 1 in binary
    ...
    for(int v = 0; v < N; v++){
      if(((1 << v) & m) == 0){ // First condition optional

        memo[u][m] = Math.min(memo[u][m], AdjMatrix[u][v] + DP_TSP(v, (1 << v) | m));
