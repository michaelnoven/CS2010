...
class BabyTree{
  ...
  public int countSubMap(String start, String end){
    ...
    // Find most left node
    Baby currentBaby = findMin(root);

    // Traverse to the right node
    while(currentBaby != null){

      if(currentBaby.babyName.compareTo(start) >= 0){
        countNames++;
        break;
      }

      currentBaby = successor(currentBaby.babyName);
...
class BabyNames {
  ...
  // Comment duplcate submission osv..
